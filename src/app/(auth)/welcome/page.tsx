...
const Page = () => {
    ...
    const { data } = useQuery({
        queryFn: async () => {
            const res = await client.auth.getDatabaseSyncStatus.$get() /* if mutation we will change it to $post() */
            return await res.json() /* type safe api fetch */
        },
        queryKey: ['get-database-sync-status'], /* react-q takes queryfn and queryKey */
        refetchInterval: (query) => {
            return query.state.data?.isSynced ? false : 1000 /* refetch the data if user not synced */
        },
    })
    ...
    return (
        <div className='flex w-full flex-1 items-center justify-center px-4'><BackgroundPattern className='absolute inset-0 left-1/2 z-0 -translate-x-1/2
        opacity-75' />
            <div className='relative z-0 flex -translate-y-1/2 flex-col items-center gap-6 text-center'>
