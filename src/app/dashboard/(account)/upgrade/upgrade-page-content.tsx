...
const UpgradePageContent = ({ plan }: { plan: Plan }) => {
    ...
    const { mutate: createChecckoutSession } = useMutation({
        mutationFn: async () => {
        ...
        <div className='max-w-3xl flex flex-col gap-8'>
            <div>
                <h1 className='mt-2 text-xl/8 font-medium tracking-tight text-gray-900'>
                    {plan === "PRO" ? "Plan:Pidgey" : "Plan:Free"}
                </h1>
                <p className='text-sm/6 text-gray-600 max-w-prose'>
                    {plan === "PRO" ? "Thank you for Feeding pidgey. Enjoy the stronger pidgey usage limits below." : "Get access to stronger Pidgey by upgrading your plan."}
                </p>
            </div>
            ...
            <p className='text-sm text-gray-500'>
                ...
                {plan !== "PRO" ? <span onClick={() => createChecckoutSession()}
                    className='underline cursor-pointer text-brand-600'>{" "}or upgrade now to increase your limit</span> : null}
            </p>
        </div>
    )
}
